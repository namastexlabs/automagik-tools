...
class SlidesConfig(GoogleWorkspaceBaseConfig):
    """Slides-specific configuration"""

    model_config = ConfigDict(
        env_prefix="GOOGLE_SLIDES_", env_file=".env", env_file_encoding="utf-8"
    )


def create_server(cfg: Optional[SlidesConfig] = None):
    """Create and configure Slides MCP server."""
    global config

    config = cfg or SlidesConfig()
    ...
    # Import Slides tools to register them
    ...
    set_enabled_tools(["slides"])
    set_enabled_tool_names(None)
    filter_server_tools(server)
    configure_server_for_http()

    logger.info("Slides MCP initialized")
...
def get_metadata():
    return {
        ...
        "tags": ["google", "slides", "workspace"],
        "config_env_prefix": "GOOGLE_SLIDES_",
    }
