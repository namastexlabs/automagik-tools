...
class DriveConfig(GoogleWorkspaceBaseConfig):
    """Drive-specific configuration"""

    model_config = ConfigDict(
        env_prefix="GOOGLE_DRIVE_", env_file=".env", env_file_encoding="utf-8"
    )


def create_server(cfg: Optional[DriveConfig] = None):
    """Create and configure Drive MCP server."""
    global config

    config = cfg or DriveConfig()
    ...
    # Import Drive tools to register them
    ...
    set_enabled_tools(["drive"])
    set_enabled_tool_names(None)
    filter_server_tools(server)
    configure_server_for_http()

    logger.info("Drive MCP initialized")
...
def get_metadata():
    return {
        ...
        "tags": ["google", "drive", "workspace"],
        "config_env_prefix": "GOOGLE_DRIVE_",
    }
