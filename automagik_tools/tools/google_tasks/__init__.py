...
config: Optional["TasksConfig"] = None


class TasksConfig(GoogleWorkspaceBaseConfig):
    """Tasks-specific configuration"""

    model_config = ConfigDict(
        env_prefix="GOOGLE_TASKS_", env_file=".env", env_file_encoding="utf-8"
    )


def create_server(cfg: Optional[TasksConfig] = None):
    """Create and configure Tasks MCP server."""
    global config

    config = cfg or TasksConfig()
    ...
    # Import Tasks tools to register them
    ...
    set_enabled_tools(["tasks"])
    set_enabled_tool_names(None)
    filter_server_tools(server)
    configure_server_for_http()

    logger.info("Tasks MCP initialized")
...
def get_metadata():
    return {
        ...
        "tags": ["google", "tasks", "workspace"],
        "config_env_prefix": "GOOGLE_TASKS_",
    }
