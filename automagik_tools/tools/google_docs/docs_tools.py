...
from automagik_tools.tools.google_workspace_core.auth.service_decorator import (
    require_google_service,
    require_multiple_services,
)
from automagik_tools.tools.google_workspace_core.core.utils import (
    extract_office_xml_text,
    handle_http_errors,
)
from automagik_tools.tools.google_workspace_core.core.server import server
from automagik_tools.tools.google_workspace_core.core.comments import (
...
async def modify_doc_text(
    ...
    """
    logger.info(
        f"[modify_doc_text] Doc={document_id}, start={start_index}, end={end_index}, text={text is not None}, formatting={any([bold, italic, underline, font_size, font_family])}"
    )
