    ...
    if tools_dir.exists():
        for tool_path in tools_dir.iterdir():
            if tool_path.is_dir() and not tool_path.name.startswith('_'):
                ...
                if any(t.get('metadata', {}).get('name') == tool_name for t in tools.values()):
                    continue
                ...
                try:
                    ...
                    if (hasattr(module, 'get_metadata') and 
                        hasattr(module, 'get_config_class') and 
                        hasattr(module, 'create_server')):
            ...
            if mount_name in mounted_tools:
                ...
                try:
                    ...
                    if hasattr(config, 'api_key') and config.api_key:
                        ...
                        status.append(f"⚠️  {metadata['name']}: Mounted but not configured")
                    else:
                        status.append(f"✅ {metadata['name']}: Mounted (no config needed)")
                except:
