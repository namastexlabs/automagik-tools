...
help: ## 🛠️ Show this help message
	...
	@echo -e "  $(FONT_PURPLE)http-dev$(FONT_RESET)        Quick development setup (localhost:8884)"
	@echo -e "  $(FONT_PURPLE)http-network$(FONT_RESET)    Network accessible setup (0.0.0.0:8884)"
	@echo ""
	...
	@echo -e "  $(FONT_GRAY)make docker-run-sse PORT=8884$(FONT_RESET)"
...
docker-run-sse: ## 🚀 Run SSE server in Docker (PORT=8884)
...
docker-compose: ## 🎼 Run multi-transport setup with docker-compose
	...
	@echo -e "$(FONT_CYAN)📡 SSE server: http://localhost:8884$(FONT_RESET)"
...
http-start: ## 🌐 Start HTTP development server (HOST=0.0.0.0 PORT=8884 TOOLS=all)
	$(call print_status,Starting HTTP development server...)
	@./scripts/deploy_http_dev.sh start $(or $(HOST),0.0.0.0) $(or $(PORT),8884) $(or $(TOOLS),all)

http-stop: ## 🛑 Stop HTTP development server (PORT=8884)
	$(call print_status,Stopping HTTP development server...)
	@./scripts/deploy_http_dev.sh stop $(or $(PORT),8884)

http-restart: ## 🔄 Restart HTTP development server
	$(call print_status,Restarting HTTP development server...)
	@./scripts/deploy_http_dev.sh restart $(or $(HOST),0.0.0.0) $(or $(PORT),8884) $(or $(TOOLS),all)

http-status: ## 📊 Check HTTP server status (PORT=8884)
	@./scripts/deploy_http_dev.sh status $(or $(PORT),8884)

http-logs: ## 📋 Show HTTP server logs (PORT=8884)
	@./scripts/deploy_http_dev.sh logs $(or $(PORT),8884)

http-dev: ## 🚀 Quick development setup (localhost:8884)
	$(call print_status,Starting HTTP development setup...)
	@./scripts/deploy_http_dev.sh dev

http-network: ## 🌐 Network accessible setup (0.0.0.0:8884)
