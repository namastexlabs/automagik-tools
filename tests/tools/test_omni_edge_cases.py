...
class TestOmniEdgeCases:
    ...
    async def test_empty_parameters(self, server):
        ...
        assert "error" not in result_data or "message required" in result_data.get("error", "")
...
class TestOmniRobustness:
    ...
    async def test_network_timeout_handling(self, server):
        ...
        with patch('automagik_tools.tools.omni.client.httpx.AsyncClient') as mock_client_class:
