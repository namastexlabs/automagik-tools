...
class TestAskToolBasics:
    ...
    def test_config_creation(self):
        ...
        assert config.planning_model == "gpt-4.1-nano"
        assert config.synthesis_model == "gpt-4.1-nano"

    def test_tool_creation(self):
        """Test tool creation"""
        config = AskConfig(
            automagik_api_key="test_key", openai_api_key="test_openai_key"
        )
...
class TestAskPlanning:
    ...
    async def test_plan_only(self, mock_config, mock_orchestrator):
        ...
        mock_planner.create_plan.return_value = Mock(
            ...
            steps=[
                Mock(
                    step_id="step_1", tool_name="list_agents", description="Get agents"
                ),
        ...
        with patch(
            "automagik_tools.tools.ask.ExecutionPlanner", return_value=mock_planner
        ):
...
class TestAskConfiguration:
    ...
    def test_config_validation(self):
        ...
        config = AskConfig(enable_ai_processing=False, safe_mode=True, debug_mode=True)
    ...
    def test_config_methods(self):
        ...
        assert (
            config.is_safe_operation("delete_everything") == True
        )  # Safe mode allows all when not checking operation type
        ...
        # Test tool priority
        assert config.get_tool_priority("safe_tool") > config.get_tool_priority(
            "other_tool"
        )
