...
class TestToolInfo:
    ...
    def test_tool_name_length(self):
        ...
        tool = ToolInfo(
            operation_id="test", tool_name="a" * 40, description="Test", category="test"
        )
...
class TestProcessedOpenAPITools:
    ...
    def test_name_mappings_consistency(self, mock_processed_tools):
        ...
        for tool in mock_processed_tools.tools:
            ...
            assert (
                mock_processed_tools.name_mappings[tool.operation_id] == tool.tool_name
            )
...
class TestOpenAPIProcessor:
    ...
    @patch("automagik_tools.tools.automagik.agent_processor.Agent")
    def test_process_openapi_spec(
        self, mock_agent_class, sample_openapi_spec, mock_processed_tools
    ):
...
class TestCreateIntelligentNameMappings:
    ...
    def test_successful_mapping_generation(
        self,
        ...
        result = create_intelligent_name_mappings(
            openapi_url="http://test.com/openapi.json", api_key="test-key"
        )
        ...
        assert (
            result[
                "get_user_session_messages_api_v1_users__user_id__sessions__session_id__messages_get"
            ]
            == "get_messages"
        )
    ...
    def test_processor_error_handling(
        self, mock_processor_class, mock_httpx_get, sample_openapi_spec
    ):
        ...
        mock_processor = Mock()
        mock_processor.process_openapi_spec.side_effect = Exception(
            "AI processing error"
        )
        mock_processor_class.return_value = mock_processor
...
class TestIntegration:
    ...
    async def test_ai_processor_integration(self, tmp_path):
        ...
        config = AutomagikAgentsConfig(
            use_ai_processor=True, openai_api_key="test-key", cache_ai_results=True
        )
        ...
        with patch(
            "automagik_tools.tools.automagik.agent_processor.OpenAPIProcessor"
        ) as mock_processor_class:
            mock_processor = Mock()
            mock_processor.process_openapi_spec.return_value = ProcessedOpenAPITools(
                tools=[], name_mappings={}, categories=[]
            )
            mock_processor_class.return_value = mock_processor
