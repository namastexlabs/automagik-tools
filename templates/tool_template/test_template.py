...
import httpx
from automagik_tools.tools.{TOOL_NAME_LOWER} import create_tool
...
async def test_get_status_resource(tool):
    ...
    result = await tool._resource_handlers['{TOOL_NAME_LOWER}://status']['handler']()
...
async def test_get_config_resource(tool, mock_config):
    ...
    result = await tool._resource_handlers['{TOOL_NAME_LOWER}://config']['handler']()
...
async def test_prompts(tool):
    ...
    assert "{TOOL_NAME}" in setup_content
