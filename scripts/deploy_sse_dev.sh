#!/bin/bash
# Development deployment script for automagik-tools SSE server
# Provides convenient commands for common deployment scenarios
...
# Show usage
show_usage() {
    echo -e "${BLUE}=== Automagik Tools SSE Development Deployment ===${NC}"
    echo ""
    echo "Usage: $0 <command> [options]"
    echo ""
    echo -e "${GREEN}Commands:${NC}"
    echo "  start [host] [port] [tools]  - Start SSE server (default: 0.0.0.0 8000 all)"
    echo "  stop [port]                  - Stop SSE server (default: 8000)"
    echo "  restart [host] [port] [tools] - Restart SSE server"
...
check_status() {
    local port="${1:-8000}"
    local pid_file="logs/sse_server_${port}.pid"
...
show_logs() {
    local port="${1:-8000}"
    local log_file="logs/sse_server_${port}.log"
...
case "${1:-}" in
    "start")
        "$SCRIPT_DIR/start_sse_server.sh" "${2:-0.0.0.0}" "${3:-8000}" "${4:-all}"
        ;;
    
    "stop")
        "$SCRIPT_DIR/stop_sse_server.sh" "${2:-8000}"
        ;;
    
    "restart")
        "$SCRIPT_DIR/stop_sse_server.sh" "${3:-8000}"
        sleep 2
        "$SCRIPT_DIR/start_sse_server.sh" "${2:-0.0.0.0}" "${3:-8000}" "${4:-all}"
        ;;
    ...
    "dev")
        ...
        "$SCRIPT_DIR/start_sse_server.sh" "localhost" "8000" "all"
        ;;
    
    "network")
        echo -e "${BLUE}Starting network server...${NC}"
        "$SCRIPT_DIR/start_sse_server.sh" "0.0.0.0" "8000" "all"
        ;;
    
    "test")
        echo -e "${BLUE}Starting test server...${NC}"
        "$SCRIPT_DIR/start_sse_server.sh" "localhost" "8001" "all"
        ;;
    
    "evolution")
        echo -e "${BLUE}Starting Evolution API server...${NC}"
        "$SCRIPT_DIR/start_sse_server.sh" "0.0.0.0" "8002" "evolution-api,evolution-api-v2"
        ;;
    
    "genie")
        echo -e "${BLUE}Starting Genie server...${NC}"
        "$SCRIPT_DIR/start_sse_server.sh" "0.0.0.0" "8003" "genie"
        ;;
